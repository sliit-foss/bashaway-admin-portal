...
const Dropdown = ({ options, onChange, ...props }) => {
  ...
  return (
    <div id={wrapperId} className={`${props.wrapperclasses || ""} w-full relative`}>
      ...
      {showItems && (
        <div
          className="absolute w-full left-0 z-10 mt-2 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none max-h-[50vh] overflow-y-auto dropdown-section"
          role="menu"
      ...
      {selectedOption !== null && selectedOption !== "undefined" && (
        ...
        >
          <AiOutlineClose
            className={`w-[1.2rem] h-[1.2rem] cursor-pointer fill-current`}
            onClick={onSelect}
            style={{ color: props.theme === "light" ? "#000" : "#F0F2F5" }}
          />
        </div>
      )}
    </div>
