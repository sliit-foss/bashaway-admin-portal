...
const Filters = ({ filters, setFilterQuery }) => {
  ...
  return (
    <div className="w-full mb-4">
      ...
      <div className={`w-full grid ${filters.length > 2 ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'} gap-x-6 lg:gap-y-2`}>
        {filtersLocalState.map((filter, index) => {
          return (
            <div key={`filter-${filter.key}-${index}`} className={`w-full h-full flex flex-col justify-center items-start`}>
