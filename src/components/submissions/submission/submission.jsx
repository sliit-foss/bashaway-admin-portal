    ...
    >
      <div className="w-full flex flex-col items-center md:items-start gap-2">
        ...
        <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-3 text-sm sm:text-base items-center">
          <Metric
            metric="Max Score"
            value={(submission.question.max_score ?? 0)?.toString().padStart(2, "0")}
          />
          <Metric
            metric="Marked"
            value={submission.graded_by || submission.automatically_graded ? <CheckCircle2 size={16} /> : <XCircle size={16} />}
          />
          <Metric
            metric="Score"
            value={(submission.score ?? 0)?.toString().padStart(2, "0")}
          />
        </div>
      </div>
